...
@MainActor
@Observable
final class RecurringIncomesViewModel {
    ...
    func update(_ item: RecurringIncome) async throws {
        let saved = try await FinanceAPI.updateRecurringIncome(
            id: item.id,
            amount: item.amount,
        ...
        let synced: RecurringIncome
        if item.active != saved.active {
            synced = try await FinanceAPI.setRecurringIncomeActive(id: item.id, active: item.active)
        } else {
            synced = saved
        }
    ...
    func setActive(_ item: RecurringIncome, active: Bool) async {
        do {
            let saved = try await FinanceAPI.setRecurringIncomeActive(id: item.id, active: active)
    ...
    func delete(_ item: RecurringIncome) async {
        let snapshot = items
        items.removeAll { $0.id == item.id }
        do {
            try await FinanceAPI.deleteRecurringIncome(id: item.id)
        } catch {
            items = snapshot
            await load()
        }
...
struct RecurringIncomesView: View {
    ...
    @State private var itemToDelete: RecurringIncome?
        ...
        NavigationStack {
            Group {
                if viewModel.isLoading {
                    ...
                    ContentUnavailableView("recurringIncome.empty.title", systemImage: "arrow.up.circle", description: Text("recurringIncome.empty.body"))
                } else {
                    List {
            ...
            .confirmationDialog(
                "recurringIncome.confirmDelete.title",
                isPresented: Binding(
...
private struct RecurringIncomeRow: View {
    let item: RecurringIncome
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text(CurrencyFormatter.format(item.amount))
                    .font(.headline)
                    .foregroundStyle(item.active ? AppColors.positive : AppColors.muted)
                Spacer()
                if item.active {
            ...
            Text(item.accountName)
                ...
                .foregroundStyle(AppColors.muted)
            if item.active {
                Text(String(format: String(localized: "recurringIncome.nextDue"), DateUtils.formatShortDate(item.nextDate)))
                    .font(.caption)
                    .foregroundStyle(AppColors.muted)
            }
            ...
            Toggle("recurringIncome.form.isRecurring", isOn: Binding(
                get: { item.active },
...
private struct RecurringIncomeEditSheet: View {
    @Environment(\.dismiss) private var dismiss

    let accounts: [Account]
    var onSave: (RecurringIncome) async throws -> Void
    ...
    init(
        ...
        accounts: [Account],
        onSave: @escaping (RecurringIncome) async throws -> Void
    ) {
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Toggle("recurringIncome.form.isRecurring", isOn: $isActive)
            ...
            .navigationTitle("recurringIncome.edit.title")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
